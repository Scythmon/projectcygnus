...
public class FamineArmorItem extends ArmorItem implements GeoItem {
    ...
    public FamineArmorItem(ArmorMaterial material, Type type, Properties pProperties) {
    ...
    public void inventoryTick(ItemStack pStack, Level pLevel, Entity pEntity, int pSlotId, boolean pIsSelected) {
        if (pLevel.getGameTime() % 20 != 0
                ...
                || !isPlayerWearingFullSet(player, ModArmorMaterials.FAMINE)
        ) return;
    ...
    public void initializeClient(Consumer<IClientItemExtensions> consumer) {
        consumer.accept(new IClientItemExtensions() {
            private FamineArmorRenderer renderer;

            @Override
            public @NotNull HumanoidModel<?> getHumanoidArmorModel(LivingEntity livingEntity, ItemStack itemStack,
                                                                   EquipmentSlot equipmentSlot, HumanoidModel<?> original) {
                if (this.renderer == null)
                    this.renderer = new FamineArmorRenderer();
